...
# 3단계: 전입예정확인서
    ...
    if st.button("✒️다음 단계로"):
        valid, error = validate_inputs(st.session_state.student_name, parent_name, student_phone, parent_phone, student_school, address, next_grade, st.session_state.move_date)
        if not valid:
            st.error(error)
            st.stop()
        try:
