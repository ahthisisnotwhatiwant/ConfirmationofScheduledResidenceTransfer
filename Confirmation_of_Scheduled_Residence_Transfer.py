...
st.markdown("""
### 사용 방법
1. **지역 및 학교 선택**: 전입 예정 지역과 학교를 선택하세요.
2. **개인정보 동의**: 동의서를 읽고 동의에 체크하세요.
3. **정보 입력**: 학생 및 법정대리인 정보를 입력하고 서명하세요.
4. **제출**: 생성된 PDF를 확인 후 제출하세요.
""")
...
def validate_inputs(student_name, parent_name, student_phone, parent_phone, address, next_grade):
    ...
    if not (re.match(phone_pattern, student_phone) and re.match(phone_pattern, parent_phone)):
        return False, "전화번호 형식이 올바르지 않습니다 (예: 010-1234-5678)."
    return True, ""
